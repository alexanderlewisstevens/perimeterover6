...
formulation that does not reduce to a hole-free polyomino.  In three
dimensions, the direct point-guard analogy fails; edge, reflex-edge, face, and
polycube guard models carry the relevant open questions.
...
Orthogonal polyhedra in 3D & No direct point-guard analogue; edge and
reflex-edge variants remain active. & The 3D branch must be read through guard
model changes rather than by copying the planar perimeter theorem. &
...
\begin{description}
  ...
  sliding-camera, sliding-transmitter, $k$-hop, polyform, face-guard, and
  2-reflex variants.  It also has explicit scope-exclusion cells for
...
12 & Is the prose synchronized with the evidence?  Regenerate the evidence
...
technical reports, or primary metadata.  The first forward and adjacent-model
sweep for the central holes, perimeter, and 3D edge/reflex-edge clusters is
now logged.  It did not add a tracked theorem that changes the main statuses.
...
sliding-transmitter visibility, $k$-hop and polyform visibility, 3D face
guards, and 2-reflex reflex-edge guards.  It also records scope-exclusion
cells for floodlight, half-plane, dispersive, contiguous, mobile,
...
search-frontier work rather than a missing-locator problem.
...
The May 23, 2026 search-log entry is the current checkpoint for recent-status
claims.  It records exact-title, forward-citation-style, arXiv/DROPS/topic,
and adjacent-model queries for the orthogonal-polygons-with-holes,
polyomino-perimeter-with-holes, and 3D orthogonal-polyhedron clusters.  The
result was status preservation rather than source expansion: the central open
and conditional cells remain open or conditional in the tracked evidence, and
the relevant-looking adjacent hits are recorded either as cited adjacent cells
or as explicit scope-exclusion cells.
