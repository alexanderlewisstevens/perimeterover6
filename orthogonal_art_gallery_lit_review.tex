...
\begin{abstract}
% Intent: Give expert readers the main status result before terminology and details.
This review organizes the art-gallery literature closest to perimeter-type
guard bounds for orthogonal objects.  In two dimensions, the
perimeter-over-six question only has invariant content after fixing a lattice
scale: it is proven for hole-free polyominoes, conditional/open for
polyominoes with holes, and open for general integral orthogonal polygons.
In three dimensions, a direct point-guard analogue of the planar theorem
fails: orthogonal polyhedra may require $\Theta(n^{3/2})$ point guards.  The
natural 3D analogues therefore shift to edge guards, reflex-edge guards, face
guards, or discrete polycube visibility models, where several Urrutia-type
...
% Intent: Fix all planar object terminology once so later sections can use it without redefinition.
...
% Intent: Resolve the user's 3D terminology question without repeating it elsewhere.
...
% Intent: Make guard-type changes explicit because several cited bounds use different guard models.
...
% Intent: Explain why perimeter-over-six is a lattice question, not a Euclidean-scaling statement.
...
\begin{enumerate}
  % Intent: Anchor the review in the classical polygon theorem and survey sources.
  ...
  % Intent: Identify the first orthogonal strengthening before moving to holes and perimeter.
  ...
  % Intent: Separate vertex-guard hole conjectures from the perimeter-over-six question.
  ...
  % Intent: Introduce the lattice models where perimeter and area become separate parameters.
  ...
  % Intent: Place the recent ortho-unit theorem as a narrow solved case of the perimeter program.
  ...
  % Intent: Explain why the 3D branch shifts from point guards to edge-like guards.
...
\section{One-Page Status Matrix}

% Intent: Tell readers how to use the matrix and why model distinctions matter.
Table~\ref{tab:status-matrix} is the fastest way to read the review.  The
...
Open at the strongest conjectured bounds; partial results for restricted
cases & Urrutia; Hoffmann--Kriegel; Zylinski
\cite{UrrutiaOpenProblems,HoffmannKriegel1996,Zylinski2006} \\
...
Integral orthogonal polygon & point & lattice perimeter $N$ &
Conjectural: $\floor{N/6}$; no proof found in this search & Diaz-Banez et al.
...
\begin{description}
  % Intent: Present coloring as the classical baseline and mark its perimeter limitation.
  ...
  % Intent: Explain the main decomposition route for orthogonal polygons with holes.
  ...
  % Intent: Identify the method behind Massberg's theorem and the precise hole-case obstruction.
  ...
  % Intent: Separate area-based polyomino theorems from perimeter-based ones.
  ...
  % Intent: Clarify that computational hardness does not negate extremal guard bounds.
  ...
  % Intent: Record the reason point guards are the wrong direct 3D analogue.
  ...
  project should not simply ask for a point-guard analogue of the planar
  perimeter theorem.

  % Intent: Point to the 3D proof tradition that remains viable.
...
\begin{description}
  % Intent: State the solved simple-orthogonal benchmark.
  ...
  % Intent: Summarize the hole results that are relevant but not perimeter results.
  ...
  % Intent: State the proven perimeter-over-six theorem in its exact model.
  ...
  % Intent: Mark the polyomino-with-holes perimeter statement as conditional rather than proven.
  ...
  % Intent: Record the strongest clean theorem for holes while distinguishing area from perimeter.
  ...
  % Intent: Explain the narrow unit-edge result and why it is stronger than P/6 there.
  ...
  % Intent: Identify the broad 2D perimeter conjecture that remains open.
  ...
  % Intent: Locate polycubes as a related discrete branch rather than the same continuous problem.
  ...
  % Intent: State the 3D point-guard obstruction in terms of vertices.
  ...
  % Intent: Summarize the edge/reflex-edge status as the main open 3D path.
...
% Intent: Establish the classical art-gallery baseline and cite the standard surveys.
...
% Intent: State the standard orthogonal vertex theorem and separate it from perimeter questions.
...
% Intent: Connect the simple-polygon theorem to later decomposition and edge-guard methods.
...
% Intent: Summarize the main orthogonal-with-holes conjectures without conflating them with P/6.
...
% Intent: Give the reader the precise reason these hole results do not answer the target question.
...
% Intent: State Massberg's theorem and its proof mechanism in the exact hole-free setting.
...
% Intent: Present the area theorem as the main proven holes result and distinguish its parameter.
...
% Intent: Identify the exact dependency that keeps the perimeter hole case open.
...
% Intent: Position the ortho-unit theorem as a solved unit-edge special case.
...
% Intent: State the broader integral conjecture as the closest published perimeter-over-six problem.
...
% Intent: Explain why continuous 3D point guarding is not the right analogue.
...
% Intent: Introduce the Urrutia-style edge and reflex-edge conjectures as the main 3D replacement.
...
% Intent: Summarize the strongest orthogonal-polyhedron partial upper bounds.
...
% Intent: Record the broader 3D edge-guard bound and the remaining gap.
...
% Intent: Mention face guards as part of the 3D model space without making them the target model.
...
% Intent: Point discrete 3D work toward polycube and polyhypercube models.
...
\section{How Open Status Is Classified}

% Intent: Define the evidence labels that will be used in the quote and problem sections.
This review uses the following labels for open-problem evidence.

\begin{description}
  % Intent: Reserve the strongest label for matching upper and lower bounds in the same model.
  \item[Proven tight.]
  A peer-reviewed theorem gives an upper bound and a matching lower-bound
  construction in the same model.

  % Intent: Distinguish named conjectures from the review's own derived directions.
  \item[Source-stated conjecture.]
  A paper, thesis, or curated open-problem page explicitly states the question
  as a conjecture or open problem.

  % Intent: Flag results that depend on an unproved intermediate statement.
  \item[Conditional theorem gap.]
  A source claims that a desired result follows if another named conjecture or
  structural lemma is true.  The downstream result should be cited as
  conditional, not as proven.

  % Intent: Keep speculative research directions useful but clearly marked.
  \item[Derived research direction.]
  The review identifies a natural next question by comparing known theorems
  and failures.  These are useful project directions, but they should not be
  cited as named conjectures unless a primary source is later found.
\end{description}

\section{Quoted Open-Problem Evidence}

% Intent: Explain why short quotes appear here and how they should be read.
...
% Intent: Show the conditional phrase that blocks the hole extension.
\begin{quote}
...
\noindent
% Intent: Interpret the Massberg quote as a dependency, not a theorem.
...
% Intent: Include the tempting hole-extension phrase while preserving the missing hypothesis.
\begin{quote}
...
\noindent
% Intent: Warn against citing the conditional conclusion by itself.
...
% Intent: Quote the current integral-perimeter conjecture in its published form.
\begin{quote}
...
\noindent
% Intent: Separate the proved ortho-unit theorem from the broader integral conjecture.
...
% Intent: Record Shermer's holes conjecture while keeping the guard model explicit.
\begin{quote}
...
\noindent
% Intent: Prevent a vertex-guard conjecture from being mistaken for a point-guard perimeter theorem.
...
% Intent: Quote the 3D point-guard obstruction.
\begin{quote}
...
\noindent
% Intent: Use the quote to justify moving from 3D point guards to edge-like guards.
...
% Intent: Quote the explicit open gap in 3D edge guarding.
\begin{quote}
...
\noindent
% Intent: Record that the edge-guard gap is source-confirmed.
...
% Intent: Include a current related discrete visibility open problem without overstating relevance.
\begin{quote}
...
\noindent
% Intent: Classify the k-hop question as adjacent rather than part of the standard model.
...
% Intent: Give readers a roadmap through the open-problem list before the details.
Problems 1--3 are closest to the perimeter-over-six question.  Problems 4--5
are the main 2D orthogonal-with-holes vertex-guard conjectures.  Problems 7--10
are the 3D orthogonal-polyhedron directions.  Each item records whether its
open status is source-stated, conditional, or derived from the literature.

\begin{enumerate}
  % Intent: State the most direct unresolved perimeter-over-six question.
  \item \textbf{Perimeter-over-six for polyominoes with holes.}

  \emph{Kind: conditional theorem gap; priority: high.}
  ...
  % Intent: Isolate the structural conjecture that would imply the hole case.
  \item \textbf{Ma{\ss}berg's maximal-rectangle packing conjecture for holes.}

  \emph{Kind: source-stated conjecture; priority: high.}
  ...
  % Intent: State the broadest current 2D lattice-perimeter conjecture.
  ...
  % Intent: Record the main vertex-guard conjecture for orthogonal polygons with holes.
  \item \textbf{Shermer's vertex-guard conjecture for orthogonal polygons with holes.}

  \emph{Kind: source-stated conjecture; priority: medium-high.}
  ...
  % Intent: Record the second major vertex-guard conjecture for holes.
  \item \textbf{Hoffmann's vertex-guard conjecture for orthogonal polygons with holes.}

  \emph{Kind: source-stated conjecture; priority: medium.}
  ...
  % Intent: Provide a fallback research direction if P/6 fails with holes.
  \item \textbf{If perimeter-over-six with holes is false, find the correction term.}

  \emph{Kind: derived research direction; priority: high if counterexamples emerge.}
  ...
  % Intent: State the central edge-guard conjecture for orthogonal polyhedra.
  \item \textbf{3D Urrutia edge-guard conjecture for orthogonal polyhedra.}

  \emph{Kind: source-stated conjecture; priority: high for 3D.}
  ...
  % Intent: State the reflex-edge conjecture and its restricted solved case.
  \item \textbf{3D reflex-edge conjecture for orthogonal polyhedra.}

  \emph{Kind: source-stated conjecture with proven restricted cases; priority: high for 3D.}
  ...
  % Intent: Include the general edge-guard gap as a broader 3D problem.
  \item \textbf{Close the general 3D edge-guard gap.}

  \emph{Kind: source-stated open problem; priority: medium-high.}
  ...
  % Intent: Emphasize that 3D progress depends on choosing the right parameter first.
  \item \textbf{Choose the correct 3D analogue of perimeter.}

  \emph{Kind: derived research direction; priority: high before theorem work.}
  ...
  % Intent: Add algorithmic questions without mixing them with extremal bound questions.
  \item \textbf{Algorithmic complexity and approximation for restricted orthogonal models.}

  \emph{Kind: mixed source-stated and derived direction; priority: medium.}
...
\section{Search Method and Source-Quality Notes}

% Intent: Make the literature-search basis auditable without interrupting the main survey.
This review was checked on May 22, 2026.  The source search prioritized
peer-reviewed theorem statements, author-posted PDFs, theses when they contain
explicit conjectures or model summaries, and curated open-problem pages by
researchers in the area.  Searches used combinations of:
\begin{itemize}
  \item \texttt{orthogonal art gallery holes};
  \item \texttt{polyomino perimeter guards};
  \item \texttt{ortho-unit polygons guarded perimeter};
  \item \texttt{integral orthogonal polygon perimeter guards};
  \item \texttt{orthogonal polyhedra edge guards};
  \item \texttt{reflex edge guards 2-reflex orthogonal polyhedra};
  \item \texttt{k-hop visibility polyominoes holes}.
\end{itemize}

% Intent: Rank source authority so readers can judge the confidence of each claim.
The review treats sources in the following order of strength:
\begin{enumerate}
  \item Peer-reviewed theorem sources with exact guard model and parameter.
  \item Theses or author PDFs when they state a conjecture, collect a known
  theorem, or give a precise proof dependency.
  \item Curated open-problem pages when they record named conjectures and no
  later proof was found.
  \item Derived questions created by comparing the known theorems and gaps.
\end{enumerate}

% Intent: Clarify that the search outcome is evidence, not a formal proof of openness.
The phrase ``no later proof was found'' is not itself a mathematical proof of
openness.  It records the outcome of this search and is included to prevent
conditional statements from being accidentally cited as established theorems.

\section{Journals and Venues to Monitor}

\subsection*{Journals}

% Intent: List journals where related discrete geometry, algorithms, and visibility work appears.
\begin{itemize}
...
% Intent: List conference venues likely to carry new results before journal publication.
\begin{itemize}
...
% Intent: Group the foundational sources that define the classical art-gallery baseline.
\begin{description}
...
% Intent: Group sources on orthogonal polygons, holes, and related hardness.
\begin{description}
...
% Intent: Group sources most directly connected to lattice perimeter and polyomino guarding.
\begin{description}
...
% Intent: Group the 3D sources by point, edge, reflex-edge, and face-guard models.
\begin{description}
...
% Intent: Group algorithmic and variant-model sources separately from extremal theorem sources.
\begin{description}
...
\begin{enumerate}
  % Intent: Give the citation rule for the proven perimeter-over-six theorem.
  ...
  % Intent: State the status of the broad integral perimeter statement.
  ...
  % Intent: Preserve the corrected 3D terminology for future writing.
  ...
  % Intent: Make the central 3D modeling recommendation explicit.
  ...
  % Intent: End with the most promising project targets.
